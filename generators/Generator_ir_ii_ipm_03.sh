    ...
    for runner in ${RUNNERS[@]}; do
        for param in ${precision[@]}; do
            for method in ${METHODS[@]}; do #Change this
                DIRC="/home/quantum/interior_point_methods/results/IR_II_IPM_precision"
                mkdir -p ${DIRC}
                if [[ "$method" -eq 1 ]]; then
                    str_method="II_IPM"
                else
                    str_method="IR_II_IPM"
                fi
