...
from ipm.ipm_methods._inexact_infeasible_IPM_SDO import inexact_infeasible_IPM_SDO
from ipm.ipm_methods._inexact_feasible_IPM_SDO import inexact_feasible_IPM_SDO
...
class SDO(Parameters):
	...
	def __init__(self, A=None, b=None, c=None):
		...
		if isinstance(A, np.ndarray) == True or isinstance(A, list) == True :
			...
			sys.exit("A is not a 3D list or array.")
		...
		if self.A.shape[1] != self.n:
			sys.exit("The dimension of matrix A and matrix C does not match.")

		if self.A.shape[2] != self.n:
			sys.exit("The dimension of matrix A and matrix C does not match.")
	...
	def primal_objective(self):
		return np.trace(np.matmul(self.c,self.x))
	...
	def primal_residual(self):
		...
		for i in range(self.m):
			r[i]=np.trace(np.matmul(self.A[i],self.x))
		return np.linalg.norm(r - self.b)
	...
	def dual_residual(self):
		...
		for i in range(self.m):
			r=r+self.y[i]*self.A[i]
		return np.linalg.norm(self.c - r - self.s)
	...
	def complementarity(self):
		return np.trace(np.matmul(self.s,self.x))
	...
	def solve(self):
		...
		if self.Params.Method == "II-IPM":
			...
			solver 						= inexact_infeasible_IPM_SDO
			...
			self.Params.Is_Quantum 		= False
			solver 						= inexact_feasible_IPM_SDO 
			...
			solver 						= iterative_refinement_SDO

			if "II-QIPM" in self.Params.Method:
				self.Params.Is_Quantum 	= True
				self.linear_optimizer 	= inexact_infeasible_IPM_SDO
				...
				self.Params.Is_Quantum 	= True
				self.linear_optimizer 	= inexact_feasible_IPM_SDO
